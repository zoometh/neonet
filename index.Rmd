...
data.c14 <- read.csv(paste0(getwd(), "/inst/extdata/c14_dataset.tsv"), sep = "\t")
# data.c14 <- data.c14[!is.na(data.c14$inTime) & !is.na(data.c14$inSpace), ]
tot.nb <- nrow(data.c14)

ws_roi <- readOGR("C:/Users/supernova/Dropbox/My PC (supernova-pc)/Documents/neonet/doc/data/wsh_roi.shp")
df_ws_roi = data.frame(ws_roi)
sp_ws_roi = SpatialPolygonsDataFrame(ws_roi, df_ws_roi)
ws_roi_atl <- readOGR("C:/Users/supernova/Dropbox/My PC (supernova-pc)/Documents/neonet/doc/data/wsh_atl.shp")
df_ws_roi_atl = data.frame(ws_roi_atl)
sp_ws_roi_atl = SpatialPolygonsDataFrame(ws_roi_atl, df_ws_roi_atl)
...
c14mater.url <- 'https://raw.github.com/zoometh/neonet/master/inst/extdata/c14_material_life.tsv'
...
# references.url <- 'https://raw.github.com/zoometh/neonet/master/inst/extdata/references.bib'
# bib <- read.bib(references.url)
# print(bib[4], style = "Bibtex")
# library(utils)
URL <- "https://raw.github.com/zoometh/neonet/master/inst/extdata/references.bib"
...
```{r previewroiatl, screenshot.force = TRUE, echo=F, warning= F, fig.align='center', fig.cap="Preview of the future NeoNet dataset, in blue, covering the Middle and Southern European Atlantic watershed"}
# leaflet(width = "60%", height = "400px") %>%
leaflet() %>%
  # setView(lng = -111.846061, lat = 36.115847, zoom = 12) %>%
  addProviderTiles(providers$Stamen.TerrainBackground, group='Terrain') %>%
  ...
  addPolygons(data = sp_ws_roi,
              ...
              opacity = .7,
              weight = 4) %>%
    addPolygons(data = sp_ws_roi_atl,
              color = "blue",
              # fillColor = 'red',
              fillOpacity = 0,
              opacity = .7,
