...
![](img/donzere-gis-dates.png){width="600"}
...
```{.r}
neo_isochr(df.c14, lbl.dates = TRUE, lbl.time.interv = TRUE)
```
...
```{.r}
neo_isochr(df.c14, selected.per = "LM", lbl.dates = TRUE, lbl.time.interv = TRUE)
```

![](img/donzere_LM-paleolithic-isochr.png){width="700"}
