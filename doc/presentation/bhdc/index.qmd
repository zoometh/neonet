---
title: "NeoNet app"
subtitle: "Radiocarbon modelling for the Late Mesolithic/Early Neolithic transition in South-Central and South-Western Europe"
author:
  - name: Thomas Huet
    affiliation: 
      - id: uo
        name: <span style="font-size:16pt">University of Oxford</span>
  - name: Niccolò Mazzucco
    affiliation: <span style="font-size:16pt">Università di Pisa</span>
  - name: Miriam Cubas Morera
    affiliation: <span style="font-size:16pt">Universidad de Alcalá</span>
  - name: Juan F. Gibaja
    affiliation: 
      - id: csic
        name: <span style="font-size:16pt">Consejo Superior de Investigaciones Científicas</span>
  - name: F. Xavier Oms
    affiliation: <span style="font-size:16pt">Universitat de Barcelona</span>
  - name: António Faustino Carvalho
    affiliation: 
      - id: ua
        name: <span style="font-size:16pt">Universidade do Algarve</span>
  - name: Ana Catarina Basílio
    affiliation: 
      - ref: ua
  - name: Elías López-Romero
    affiliation:
      - ref: csic
title-slide-attributes:
    data-background-image: "https://raw.githubusercontent.com/zoometh/neonet/master/doc/img/img/app_neonet_map_ortho_clust"
    data-background-size: "100%"
    data-background-opacity: "0.3"
format:
  revealjs: 
    css: https://zoometh.github.io/thomashuet/css/quarto.css
    auto-stretch: false
    code-fold: false
    code-summary: "<small>Code</small>"
    slide-number: true
    chalkboard:
      theme: "whiteboard"
      color: ['rgba(255, 0, 0, 0.3)'] # red
    scrollable: true
    preview-links: auto
    reference-location: document
    footnotes-hover: true
    # self-contained: true
    # embed-resources: true
    logo: https://raw.githubusercontent.com/zoometh/neonet/master/doc/img/neonet.png
    footer: "<em>Big Historical Data Conference</em> <img src='https://raw.githubusercontent.com/zoometh/neonet/master/doc/presentation/bhdc/img/bhdc_avatar.png' style='height: 35px;vertical-align: middle;'> <https://bhdc.earth/> 22-25 November 2023, Jena (Germany)"
    margin: 0
    width: 1200
---

::: {.notes}
Radiocarbon dates are among the most common and widely shared archaeological data across regional specificities. Accompanied by archaeological contexts, these dates are often utilized to develop large-scale models of human activities and to categorize cultural groups over time and space. The NeoNet project brings together several archaeologists from different geographic areas, focusing on the transition between the last hunter-gatherers and the early farmers. As part of this project, we have compiled and curated a substantial dataset of radiocarbon dates, which can be accessed through an R Shiny online app, the NeoNet app. Today, we want to explain how our method tackles research questions and follows the FAIR principles.

:::

## {data-background-iframe="http://shinyserver.cfs.unipi.it:3838/C14dev" background-interactive="true"}

::: {.notes}
NeoNet app is an opensource R Shiny interactive web app which facilitates their study by providing selection tools for spatial information, chronology, and date accuracy. The map interface offers a mobile and multi-scalar window to study the dynamic of last hunter-gathers / early farmers frontiers <br>
NB: prepare the map: Global view / clustered / ortho ; Explode cluster / ungroup / OSM
:::


## {}

<div class="img">
![](img/watersheds-ortho.png){width="1100"}
</div>

::: {.notes}
To facilitate dataset curation, it has been divided into two parts based on river basins: NeoNet Atl, focusing on the Atlantic region (in blue), and NeoNet Med, dedicated to the North Central-Western Mediterranean Basin (in red). The decision to partition by river basins is grounded in two primary considerations: it allows for the systematic breakdown, or hierarchical decomposition, of river basins into interconnected and nested components; watersheds are crucial as spatial analysis units, facilitating mathematical modeling that incorporates key concepts such as catchment area, thalweg (shortest path), and natural boundaries.
:::

## {}

The NeoNet Atl dataset

```{r}
library(dplyr)
library(DT)
neonet_med <- "https://digitallib.unipi.it/fedora/objects/mag:2627/datastreams/MMb1a4a927461fdd822e923821b8d92371/content"
df.c14 <- read.csv2(neonet_med, sep = "\t", encoding = "UTF-8")
font.size <- "8pt"
df.c14[,!names(df.c14) %in% c("tpq", "taq")] %>% 
  DT::datatable(
    width = "100%",
    rownames = FALSE,
    options=list(
      lengthMenu = list(c(10, 50, -1), 
                        c('10', '50', 'All')),
      paging = T,
      initComplete = htmlwidgets::JS(
        "function(settings, json) {",
        paste0("$(this.api().table().container()).css({'font-size': '", font.size, "'});"),
        "}")
    ) 
  )
```


::: {.notes}
*Kervouric*. These datasets have been limited to essential radiocarbon date information: the date, period, location, archaeological unit, type of material dated, and bibliographical references. NeoNet Atl and Med include approximately 4,000 radiocarbon dates from around 3,000 distinct archaeological layers, along with about 800 sites mentioned in bibliographical references. These dates and archaeological contexts are backed by roughly 600 different bibliographical sources.
:::


## Atl
`id00164_doc_elencoc14.tsv`

<div class="row">
<div class="column50">![](img/figure2.png)
<div class="captiontext">Summed probability densities (SPD)</div>
</div>
<div class="column50">![](https://raw.githubusercontent.com/zoometh/neonet/master/results/atl_test1.png){height="700"}
<div class="captiontext">Distribution of dates by periods</div>
</div>

</div>

::: {.notes}
NeoNet Atl. We are in the process of publishing this dataset as a data paper, and it will be deposited on a university server.
:::



## Med
`id00140_doc_elencoc14.tsv`

<div class="row">
<div class="column50">![](https://raw.githubusercontent.com/zoometh/neonet/master/doc/img/neonet_med_spd.png)
<div class="captiontext">Summed probability densities (SPD)</div>
</div>
<div class="column50">![](https://raw.githubusercontent.com/zoometh/neonet/master/results/neo_datasum_by_periods.png){height="700"}
<div class="captiontext">Distribution of dates by periods</div>
</div>
</div>


::: {.notes}
NeoNet Med has been published in the *Journal of Open Archaeological Data*. The data paper offers a comprehensive account of the data collection process, including steps, quality control, metadata, and other essential details. Additionally, it provides valuable insights into how the dataset can be effectively reused for various purposes.
:::



## {}

<div class="img">
![](img/watersheds-climate.png){width=1000px}
</div>
<div class="captiontext">Beck et al. 2018, Present and future Köppen-Geiger climate classification maps at 1-km resolution, Nature Scientific Data[^1]
</div>

[^1]: My note

::: {.notes}
I'd like to demonstrate how the app functions with an example: examining the influence of climates on the Neolithic expansion and the potential impact of varying ecological barriers on the spread of farming. The app includes the current Koppen Climate Classification as a basemap.
:::


## {}

<div class="img">
![](img/donzere-gis.png){width="1000"}
</div>
<div class="captiontext">Beck et al. 2018, Present and future Köppen-Geiger climate classification maps at 1-km resolution, Nature Scientific Data
</div>

::: {.notes}
For instance, the Défilé de Donzère is a narrow pass in the Rhone valley, extending from North to South, located between the Alps in the east and the Massif Central in the west. In this area, the southern part has a Mediterranean climate (`Csa` - Temperate, dry and hot summer), while the northern part experiences a continental climate (`Cfb` - Temperate, no dry season, warm summer).
:::

## {}

<div class="img">
![](img/donzere-view.png){width=1200px}
<div class="captiontext">View of Défilé de Donzère and the Rhône River from the south on Google Earth./div>


::: {.notes}
View from the South of the Défilé de Donzère and the Rhône river
:::

## {}

<div class="img"><center>
![](img/donzere-app-north-and-south.png){width="900"}
</div>


::: {.notes}
An user create a selection window around the Défilé, specifically choosing dates from the Late Mesolithic (LM) and Early Neolithic (EN) periods, with a standard deviation (SD) of less than 130. Applying these filters, the dataset will contain 70 sites and 353 dates. These dates can be calibrated on the fly by selecting one of them and accessing the calib (calibration) panel.
:::

## {}

<div class="img"><center>
![](img/donzere-app-north-and-south-calib-1.png){width="900"}
</div>

::: {.notes}
Dates can be aggregated and seriated by periods (shown here), sites and periods, archaeological layer (PhaseCode), or plotted individually. However, a large number of dates will overwhelm the R Shiny server and may result in an error in this panel. This is one of the limitations of R for managing big data.
:::



## {}

<div class="img"><center>
![](img/donzere-app-north-and-south-calib.png){width="900"}
</center></div>

::: {.notes}
Here by sites and period. Graphics can be exported using a download button
Export the plot
:::


## {}


<div class="img"><center>
![](img/donzere-app-north-and-south.png){width="900"}
</center></div>


::: {.notes}
Returning in the map panel. The selected area and the selected dates can also be exported, both in GeoJSON files, using two other export buttons on the top left of the map panel
:::

## {}

```{r}
library(sf) ; library(dplyr) ; library(leaflet)
dirIn <- "https://raw.githubusercontent.com/zoometh/neonet/main/doc/presentation/bhdc/data/"
```

:::: {.columns}

::: {.column width="50%"}

<span class= smalltext><sup>`neonet-data-2023-10-22-select-aera.geojson`</sup></span>

```{r}
df <- st_read(paste0(dirIn, "neonet-data-2023-10-22-select-aera.geojson"), quiet = T)
leaflet(df, width = "500px") %>%
  addTiles() %>% 
  addPolygons()       # Change weight as needed
```

:::

::: {.column width="50%"}

<span class= smalltext><sup>`neonet-data-2023-10-22.geojson`</sup></span>

```{r}
df <- st_read(paste0(dirIn, "neonet-data-2023-10-22.geojson"),  quiet = T)
leaflet(df, width = "500px") %>%
  addTiles() %>% 
  addCircleMarkers()       # Change weight as needed
```

:::

::::


::: {.notes}
Here it's a leaflet view of these exported file. The latter can undergo a secondary curation and subsetting process in a GIS or any other appropriate application <br>
Date that have been removed: 
- Grotte de Camprafaud, lyer 20,Gif-3077
- Abri de la Font-des-Pigeons, Escalon C5-F5-C6F6, KN-182
- Petites Baties, sect. 4, Beta-103872
- Mas Neuf, Erl-11802
- Pontcharaud 2, c.3, Ly-7625
- Clos de Poujol, c. Sn-Sn1, Ly-2138 (OxA)
:::

## {}

<img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-github.png' style='height: 40px;vertical-align: middle;'><img src='https://raw.githubusercontent.com/zoometh/neonet/master/doc/img/logo_nn_pkg.png' style='height: 50px;vertical-align: middle;'> <span class= smalltext><https://github.com/zoometh/neonet> </span>

:::: {.columns}

::: {.column width="30%"}

![](img/github-neonet-functions-1.png){width="700" style='horizontal-align: middle;'}

:::

::: {.column width="70%"}

```{.r}
neo_datasum(df.c14, 
            info = c("maps"),
            roi = "https://github.com/zoometh/neonet/blob/main/doc/data/wsh_atl.geojson")
```

<div class="img"><center>
![](https://raw.githubusercontent.com/zoometh/neonet/main/results/atl_test.png){width="500" style='horizontal-align: middle;'}
</center></div>


```{.r}
neo_spd(df.c14 = df.c14)
```

<div class="img"><center>
![](img/figure2.png){width="500" style='horizontal-align: middle;'}
</center></div>

```{.r}
neo_isochr(df.c14 = "neonet-data-2023-10-22.geojson")
```

<div class="img"><center>
![](img/donzere_EN-neolithic-isochr.png){width="600" style='horizontal-align: middle;'}
</center></div>

```{.r}
neo_isochr(df.c14 = "neonet-data-2023-10-22.geojson", 
           selected.per = "LM")
```

<div class="img"><center>
![](img/donzere_LM-paleolithic-isochr.png){width="600" style='horizontal-align: middle;'}
</center></div>

```{.r}
neo_strat(inData = "Roc du Dourgne_2023-07-30.csv")
```

<div class="img"><center>
![](https://raw.githubusercontent.com/zoometh/neonet/main/doc/img/app-strati-diag-dourgne-period.png){width="450" style='horizontal-align: middle;'}
</center></div>


:::

::::

::: {.notes}
The Shiny app also includes documentation and other functions for calculating various features, including:

* SPDs (Summed Probability Distributions)
* Isochrones (considered an effective way to visually represent the diffusion in time and space within a single non-animated map)
* Harrix matrices
:::

## {}

<img src='https://raw.githubusercontent.com/zoometh/thomashuet/master/img/app-github.png' style='height: 40px;vertical-align: middle;'><img src='https://raw.githubusercontent.com/zoometh/neonet/master/doc/presentation/bhdc/img/c14bazaar-logo.png' style='height: 50px;vertical-align: middle;'> <span class= smalltext><https://github.com/ropensci/c14bazAAR></span>

:::: {.columns}

::: {.column width="30%"}

![](img/github-c14bazaar-functions-1.png){width="700" style='horizontal-align: middle;'}

:::

::: {.column width="70%"}

![](img/github-c14bazAAR-dbs.png){width="700" style='horizontal-align: middle;'}

:::

::::


::: {.notes}
NeoNet is one of the numerous existing radiocarbon datasets. That's why we have developed a *getter* function within the C14bazaar package for the dataset's to be harvestable.
:::


## Conclusion

* App and dataset of 14C dates for the **Mesolithic / Neolithic transition** in **South-Central and South-Western Europe**
* Open Data, Open Source, Open Access ➡️ **Open Science**
* Scalable, incrementable, reusable, analyzable ➡️ **Reproducibility**

::: {.notes}
* Neonet offers an App and curated dataset of radiocarbon dates for the Mesolithic / Neolithic transition, currently in South-Central and South-Western Europe
* The dataset is OD, the app and management functions are OSrc and OA. Compliant with OSci
:::

## Perspectives

<center>

![](https://raw.githubusercontent.com/zoometh/neonet/master/doc/img/panel_map_export_todo.png)

</center>

* Enhancing data and data re-usability
* Extension of the dataset to other river basins
* Build sites' Harris matrices and run Bayesian with this *prior* information

::: {.notes}
Perspectives are many:

* We need IT development to allow to study different geographical windows at the same time (here at the South and the North of the Defile)
* FAIRability can still be improved
* Watersheds partition is highly modular (hierarchical decomposition) and interactivity offer a great mean for reproductibility and robust assumptions.
* We would like to construct the Harris matrices of different site and run Bayesian statistics to better the modeling
:::

## Credits


* <span class= smalltext>"Tools, Techniques and Specialists: the keys to understand the Mesolithic-Neolithic transition in Mediterranean Europe" (PID2020-112513RB-I00) financiado/a por MCIN/AEI/ 10.13039/501100011033 </span>

* <span class= smalltext>"Origen, desarrollo y consolidación del neolítico en el ámbito Mediterráneo (RED2018-102382-T)" Redes de Investigación, 2020-2021</span>

* <span class= smalltext>Conexiones en el Sur de la Península durante el Neolítico (RED2022-134436-T)</span>

![](https://raw.githubusercontent.com/zoometh/neonet/master/doc/img/inst-gobierno-de-espana.png){height="80"} ![](https://raw.githubusercontent.com/zoometh/neonet/master/doc/img/inst-agencia-investigacion.png){height="80"} ![](https://raw.githubusercontent.com/zoometh/neonet/master/doc/img/inst-csic.png){height="80"}
</p>



