---
title: "NeoNet app"
subtitle: "Radiocarbon modelling the Late Mesolithic/Early Neolithic transition in South-Central and South-Western Europe"
author:
  - name: Thomas Huet
    affiliation: 
      - id: uo
...
::: {.notes}
The first part of this dataset, the NeoNet Med, covering the North Central-Western Mediterranean Basin, has been publish in the *Journal of Open Archaeological Data* 1-year ago. The data collection is there described in terms of steps, quality control, metadata, potential reuse, etc.
:::
...
::: {.notes}
The Défilé de Donzère is a narrow pass of the Rhone valley between Alps (east) and Massif central (west) and Mediterranean climate (south: Csa  Temperate, dry summer, hot summer) continental (north: Cfb  Temperate, no dry season, warm summer )
:::
...
::: {.notes}
A selection window can be draw around the Defile, and dates be subset to only get those of LM and EN, with SD < 130. Once done, the dataset counts 70 sites and 353 dates. The latter can be calibrate on the fly by selecting one of them and opening the calib panel.
:::
...
::: {.notes}
These files can be curated and subset in a GIS or any other app.
rm: 
...
::: {.notes}
The Shiny app comes with a serie of functions to model data such as SPD plot or Harris Matrices. For example, the neo_isochr() function creates isochrones contours by interpolation of calibrated radiocarbon dates using, for EN, the minimum median in each site (that is to say the earliest EN) and, for LM, the maximum median in each site (that is to say the latest LM)
:::
...
* Extension of the dataset to river basins
* Build sites' Harris matrices to perform Bayesian analysis on-the-fly
